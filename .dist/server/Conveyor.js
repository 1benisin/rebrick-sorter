...
class Conveyor {
    ...
    findPreviousPart(sorter) {
        const prevSorterPart = this.partQueue.reduce((acc, p) => {
            if (p.sorter === sorter)
                return p;
            return acc;
        }, null);
    ...
    logPartQueue() {
        ...
        const partQueue = this.partQueue.map((p) => ({
            ...
            initialTime: (0, utils_1.getFormattedTime)('min', 'ms', p.initialTime),
            moveTime: (0, utils_1.getFormattedTime)('min', 'ms', p.moveTime),
            moveFinishedTime: (0, utils_1.getFormattedTime)('min', 'ms', p.moveFinishedTime),
            jetTime: (0, utils_1.getFormattedTime)('min', 'ms', p.jetTime),
        }));
        console.log('partQueue:', partQueue);
    }
    logSpeedQueue() {
        const speedQueue = this.speedQueue.map((s) => ({ speed: s.speed, time: (0, utils_1.getFormattedTime)('min', 'ms', s.time) }));
    ...
    conveyorOnOff() {
        ...
        const arduinoDeviceCommand = {
            ...
            command: arduinoCommands_type_1.ArduinoCommands.CONVEYOR_ON_OFF,
        };
    ...
    clearActions() {
        ...
        this.partQueue.forEach((p) => {
            if (!!p.moveRef)
                clearTimeout(p.moveRef);
            if (!!p.jetRef)
                clearTimeout(p.jetRef);
        });
    ...
    filterQueues(sorterBinPositions) {
        // -- filter partQueue
        ...
        this.partQueue.forEach((p, i) => {
            ...
            if (!!p.jetTime && p.jetTime < Date.now())
                lastPartJettedIndex = i;
        });
        ...
        const earliestPartTime = this.partQueue.reduce((acc, p) => {
            if (p.initialTime < acc)
                return p.initialTime;
            return acc;
        }, Date.now());
    ...
    addPartToEndOfQueue(part) {
        this.partQueue.push(part);
    }
    insertSpeedChange({ startSpeedChange, newArrivalTime, oldArrivalTime, slowDownPercent, }) {
        ...
        if (newArrivalTime < Date.now() || oldArrivalTime < Date.now())
            console.error(`insertSpeedChange: time is in the past: ${Date.now()}, ${startSpeedChange}, ${newArrivalTime}, ${oldArrivalTime}`);
    ...
    prioritySortPartQueue() {
        ...
        this.partQueue = this.partQueue.map((part) => {
            if (!part.defaultArrivalTime) {
                const arrivalTime = (0, hardwareUtils_1.findTimeAfterDistance)(part.initialTime, this.jetPositions[part.sorter] - part.initialPosition, [{ time: part.initialTime, speed: this.defaultConveyorSpeed, ref: setTimeout(() => { }, 0) }]);
    ...
    scheduleConveyorSpeedChange(speed, atTime) {
        ...
        return setTimeout(() => {
            const arduinoDeviceCommand = {
                arduinoPath: this.arduinoPath,
                command: arduinoCommands_type_1.ArduinoCommands.CONVEYOR_SPEED,
