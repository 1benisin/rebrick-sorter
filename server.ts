...
import { eventHub as backendHub, AllEvents, BackToFrontEvents, FrontToBackEvents } from './server/eventHub';
...
app.prepare().then(async () => {
  ...
  const cleanupOldListeners = () => {
    Object.values(BackToFrontEvents).forEach((event) => {
      if (listeners[`B->F:${event}`]) {
        backendHub.offListener(event, listeners[`B->F:${event}`]);
  ...
  const setupSocketListeners = (socket: any) => {
    ...
    Object.values(BackToFrontEvents).forEach((event) => {
      ...
      backendHub.onEvent(event, listeners[`B->F:${event}`]);
    });
    ...
    Object.values(FrontToBackEvents).forEach((event) => {
      ...
      socket.on(event, (data: any) => {
        ...
        backendHub.emitEvent(event, data);
      });
    });
    ...
    socket.on(FrontToBackEvents.INIT_HARDWARE, async (data: any) => {
      ...
      try {
        await hardwareController.init(data);
