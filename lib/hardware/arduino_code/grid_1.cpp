...
#define GRID_DIMENSION 16 // how many bins in each direction

// Can be figured out by positioning sorter on grid and homing - will print number of steps
#define X_OFFSET 40 // 10 // how many steps from end stop to the first row of bins
#define Y_OFFSET 10 // 10 // how many steps from end stop to the first row of bins
#define X_STEPS_TO_LAST  7920 // 3960 // how many steps from origin to the last row of bins 
#define Y_STEPS_TO_LAST  7820 // 3930 // how many steps from origin to the last row of bins

#define ACCELERATION 6500 // 1200          // The acceleration value for the stepper motors
                                  // Higher values result in quicker acceleration and deceleration
                                  // Lower values result in smoother, gradual changes in speed

#define LINEAR_ACCEL_STEPS 50     // The number of steps for linear acceleration
                                  // Higher values result in a longer linear acceleration phase, creating smoother acceleration
                                  // Lower values result in a shorter linear acceleration phase, creating faster acceleration

#define USE_JUMP_START false      // The ramp step for the jump start
#define JUMP_STEPS 3             // The ramp step for the jump start
                                  // Higher values result in a slower initial speed when starting from standstill, creating smoother starts
                                  // Lower values result in a faster initial speed, creating faster starts

#define HOMING_SPEED 1000 // 4000          // The speed for the homing procedure
                                  // Higher values result in a faster homing speed
                                  // Lower values result in a slower homing speed

#define SPEED 175 // 700                  // The maximum speed for the stepper motors
                                  // Lower values result in faster movement
                                  // Higher values result in slower movement
                                  // Note: This speed may not be reached due to other factors
...
void setup()
{
  ...
  if (xStepper) {
    ...
    // xStepper->setLinearAcceleration(LINEAR_ACCEL_STEPS);
    // if (USE_JUMP_START) {
    //   xStepper->setJumpStart(JUMP_STEPS);
    // }
  }
  ...
  if (yStepper) {
    ...
    // yStepper->setLinearAcceleration(LINEAR_ACCEL_STEPS);
    // if (USE_JUMP_START) {
    //   yStepper->setJumpStart(JUMP_STEPS);
    // }
  }
...
void processMessage(char *message) {// check action of message
  switch (message[0]) {
